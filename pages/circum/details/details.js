...
Page({
  ...
  onShareAppMessage: function() {
    let that = this;
    util.request(api.businessShares, {
      id: that.data.id
    }).then(function (res) {});
    return {
      imageUrl: that.data.detailsData.picUrls[0],
      title: that.data.detailsData.title,
      path: '/pages/index/index?pageUrl=/pages/circum/details/details:id=' + this.data.id + util.shareInvite()// :代表?,;代表&，首页会做跳转处理
    }
  },
  getData: function () {
    let that = this;
    ...
    util.request(api.businessdetail, {
      id: that.data.id
    })
      .then(function (res) {
        wx.hideLoading();

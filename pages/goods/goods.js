...
Page({
  ...
  onShareAppMessage: function () {
    const { id, presellId } = this.data
    let that = this
    let url = `/pages/goods/goods?id=${id}&presellId=${presellId}`
    url = encodeURIComponent(url)
    let path = `/pages/homepage/homepage?url=${url}${util.shareInvite()}`
    return {
      imageUrl: that.data.goods.shareUrl,
      title: that.data.goods.title,
      path
      // path: '/pages/homepage/homepage?pageUrl=/pages/goods/goods:id=' + this.data.id + util.shareInvite()// :代表?,;代表&，首页会做跳转处理
    }
  },
  ...
  picPreviewShow: function () {
    let that = this
    let userId = wx.getStorageSync('userInfo').id
    if (userId) {
      util
        .request(api.GoodsShare, {
          id: that.data.id,
          presellId: that.data.presellId,
          userId
        })
        .then(function (res) {
          if (res.errno === 0) {
            that.setData({
              shareImage: res.data,
              picPreviewHidden: !that.data.picPreviewHidden,
      ...
      this.setData({
        picPreviewHidden: !this.data.picPreviewHidden,
        actionSheetHidden: !this.data.actionSheetHidden
      })
    }
  },
  picPreviewHide: function () {
    this.setData({
      picPreviewHidden: !this.data.picPreviewHidden
    })
  },
      ...
      .then(function (res) {
        if (res.errno === 0) {
          ...
          if (res.data.groupon.length > 0) {
            res.data.groupon[0].checked = true
            that.data.countdownEndTime = res.data.groupon[0].expireTime
            that.countDown()
          }
          ...
          if (!!res.data.info.video) {
            video = res.data.info.video.split(',').map((item) => ({
              src: item,
              type: 'video'
            }))
          }
          ...
          let gallery = []
          if (res.data.info.gallery.length > 0) {
            gallery = res.data.info.gallery.map((item) => ({
              src: item,
              type: 'image'
            }))
            viewContent = [...video, ...gallery]
          } else {
            viewContent = [...video, { src: res.data.info.picUrl, type: 'image' }]
          }
          ...
          that.setData({
            funBuyGoods: res.data.funBuyGoods,
            funBuy: res.data.funBuy ? res.data.funBuy : false,
            soldNumber: res.data.soldNumber,
            goods: {
              ...res.data.info,
              viewContent
            },
            ...
            checkedSpecPrice:
              res.data.groupon.length == 0
                ? res.data.info.onlinePrice
                : res.data.info.retailPrice - res.data.groupon[0].discount,
  ...
  addFast: function () {
    var that = this
    if (this.data.goods.acStatus === 98 || this.data.goods.acStatus === 99) {
      util.showErrorToast('赠品无法购买')
      return
    }
    if (this.data.openAttr == false && this.data.funBuyGoods == false) {
      //打开规格选择窗口
      this.setData({

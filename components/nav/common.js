...
Component({
  ...
  methods: {
    ...
    changeCommunity: function (e) {
      let that = this;
      util.request(api.CommunityActive, { id: e.currentTarget.dataset.id }, 'POST').then(function (res) {
        if (res.errno === 0) {
          that.getData();
          wx.setStorageSync('community', that.data.list[e.currentTarget.dataset.idx]);
        } else {
          wx.showToast({
            title: res.errmsg,
            icon: 'none'
